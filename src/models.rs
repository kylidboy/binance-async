...
pub enum AccountAndSymbolPermission {
    Spot,
    Margin,
    Leveraged,
}
...
impl std::fmt::Debug for Bids {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{{p:{}, q:{}}}", self.price, self.qty)
    }
}
...
fn get_value(row: &[Value], index: usize, name: &'static str) -> StdResult<Value, BinanceApiError> {
    Ok(row
        .get(index)
        .ok_or_else(|| BinanceApiError::KlineValueMissingError(index, name))?
        .clone())
}
...
pub(crate) mod string_or_float_opt {
    ...
    {
        match value {
            Some(v) => crate::models::string_or_float::serialize(v, serializer),
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(Some(crate::models::string_or_float::deserialize(
            deserializer,
        )?))
    }
}
